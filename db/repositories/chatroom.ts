...
export const chatRoom = z.object({
  ...
  kickedUserIds: z.array(z.string()),
})
...
export class ChatRoomRepository extends BaseRepository<ChatRoomModel> {
  ...
  async create(adminId: string, dto: ChatRoomDTO) {
    ...
    const document = this.initDocument({ ...candidate, adminId, kickedUserIds: [], connectedUserIds: [adminId] })
  ...
  async joinOne(userId: string, name: string) {
    ...
    if (!chatRoom)
      return null
    if (chatRoom.kickedUserIds.includes(userId))
